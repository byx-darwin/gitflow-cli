...
调用 `gitflow-cli issue view` 获取目标 Issue 的完整信息：

```bash
gitflow-cli issue view <issue-number>
```
...
调用 `gitflow-cli issue comment` 将分析报告作为评论发布到 Issue：

```bash
gitflow-cli issue comment <issue-number> --body-file /tmp/issue-analysis.md
```
...
gitflow-cli issue comment 42 --body-file /tmp/issue-analysis.md
rm -f /tmp/issue-analysis.md
```
...
gitflow-cli issue comment 15 --body-file /tmp/issue-analysis.md
rm -f /tmp/issue-analysis.md
```
