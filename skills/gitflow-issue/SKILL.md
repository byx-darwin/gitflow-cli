...
封装 `gitflow issue` 命令族，用于在 GitHub/GitLab/GitCode 等平台上管理 Issue。
...
### `gitflow issue reopen`

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `<number>` | int | 是 | Issue 编号 |

### `gitflow issue comment`
...
```bash
gitflow issue create --title "Fix login redirect loop" --body "Auth middleware 缺少重定向检查" --label bug --label high-priority --assignee alice
```

### 列出所有已关闭的 bug 类 Issue

```bash
gitflow issue list --state closed --label bug --limit 50
```
...
gitflow issue comment 42 --body "已复现，正在排查 middleware 逻辑"
```

### 关闭 Issue 并移除标签

```bash
gitflow issue close 42
gitflow issue label 42 --remove high-priority
```
