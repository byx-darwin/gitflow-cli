...
description: gitflow-cli 全流程开发编排 — 从需求澄清到代码交付的三阶段闸门驱动工作流，指挥 gitflow-cli 和 Superpowers 协同完成完整开发周期
---

# gitflow-workflow 全流程编排

顶层指挥中心，按照三阶段闸门驱动完整的开发生命周期。编排层只做指挥，不直接执行操作 —— 所有平台交互通过 `gitflow` CLI 完成，所有本地开发循环通过 Superpowers 完成。
...
| subagent-dev | ✅ 必须 | ⚠️ 可选 | 简单修复可直接开发 |
| **TDD** | ✅ 必须 | ✅ **必须** | 都需要回归测试 |
| code-review | ✅ 必须 | ✅ 必须 | 都需要审查 |
| quality | ✅ 必须 | ✅ 必须 | 都需要质量检查 |
| PR | ✅ 必须 | ✅ 必须 | 都需要 PR 流程 |

## 阶段总览

```
Phase 1: 需求澄清          Phase 2: 开发实现          Phase 3: 执行
────────────────────────────────────────────────────────────────────────
Superpowers:               Superpowers:               Superpowers:
  brainstorming              writing-plans              subagent-driven-development
gitflow-issue-create         TDD (per task)
gitflow-issue-review         subagent-dev
                             requesting-review
     ↓                             ↓                    ↓
产出: Issue URL              产出: 全绿原子任务清单      产出: 已完成的代码实现
      需求分析报告
```
...
- ⚠️ `superpowers:subagent-driven-development` - **可选**（简单修复可直接开发）
- ✅ `superpowers:test-driven-development` - **必须**（先写回归测试）
- ✅ `superpowers:requesting-code-review` - **必须**（代码审查）
...
```
🔒 闸门未通过：不允许进入执行阶段。所有原子任务必须标记为 done。
