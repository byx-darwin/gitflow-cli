...
2. Create the contract file at `.cache/workflows/active/<workflow_id>.json`
...
single source of truth for the workflow's state. Sub-skills read from and
write to the contract via the orchestrator — they never manage it themselves.

## Sub-Skill Invocation Rules

| Rule | Description |
|------|-------------|
| **Call and Return** | After invoking a sub-skill, the orchestrator MUST resume execution at the next step in the current Phase. Sub-skills do NOT chain to other skills. |
...
| **Evidence Before Gate** | A gate check MAY NOT pass until all required evidence fields are populated in the contract. |
| **No Implicit Completion** | A Phase is complete ONLY when the orchestrator explicitly sets `status = "complete"` in the contract. Sub-skill completion ≠ Phase completion. |

## Red Flags — STOP and Reassert Orchestrator Control

If any of these occur, **immediately**: read the contract, determine current phase, resume from next step.

| Red Flag | What It Means |
|----------|---------------|
| About to invoke `brainstorming` without a contract file | **STOP** — create contract first |
| `brainstorming` finishes and starts invoking `writing-plans` | **STOP** — interrupt, return to orchestrator, execute `gitflow-issue-create` |
| About to skip `gitflow-issue-create` or `gitflow-issue-review` | **STOP** — these are MANDATORY in Phase 1 |
| About to advance to next Phase without updating contract evidence | **STOP** — update contract first |
| User says "just write the code, skip the workflow" | **CHECK** — is this Scenario C? If no contract exists, refuse and start Phase 1 |
| About to let a sub-skill chain to another sub-skill | **STOP** — sub-skills return to orchestrator, they do not chain |

## Rationalization Table

| Excuse | Reality |
|--------|---------|
| "brainstorming will handle the Issue creation" | No — brainstorming's terminal state is `writing-plans`, not Issue creation. Orchestrator must do it. |
| "Contract can be created later" | No — contract MUST exist before any sub-skill is invoked. It is the single source of truth. |
| "User just wants to discuss, no need for full workflow" | If they invoked `/gitflow-workflow`, run the workflow. Discussion-only is not what they asked for. |
| "Issue review is optional, I can skip it" | No — `gitflow-issue-review` is MANDATORY in both full and fast modes. |
| "Brainstorming already asked clarifying questions, Phase 1 is done" | No — brainstorming is ONE step in Phase 1. Issue list, Issue create, Issue review are separate mandatory steps. |
| "The requirement is clear, skip to Phase 3" | This is Scenario C. Check contract: if `phases.2.evidence.spec_path` is empty, refuse and go to Phase 2. |
...
**When NOT to Use:**
- User wants a quick fix without workflow overhead → use `gitflow-commit` directly
- User wants to review an existing PR → use `gitflow-pr-review`
- User wants to discuss architecture without implementation → use `superpowers:brainstorming` directly
- User explicitly says "don't create an Issue" → respect that, do NOT invoke this skill
...
```
[Start] → Contract Bootstrap → Phase 1 → [Gate 1→2] → AUTO → Phase 2 → [Gate 2→3] → PAUSE → Phase 3 → [Gate 3→4] → AUTO → Phase 4 → [Archive] → [Complete]
```
...
|-----------------|---------|----------|
| Start → Bootstrap | Skill invoked | **Create contract file** |
| Bootstrap → Phase 1 | Contract created | **AUTO-ADVANCE** |
...
**Entry condition:** none (but contract MUST exist — see Core Rule)
**Exit condition:** contract `phases.1.status = complete`
**Auto-advance:** true (on gate pass)

**Execution Steps:**

1. **[AUTO]** Bootstrap — Create Contract (see "Contract Operations API" → Create Contract)
   - Initialize `.cache/workflows/active/<workflow_id>.json`
   - Set `mode` (full/fast), `title`, `current_phase = 1`
   - Set `phases.1.status = "in_progress"`

2. **[AUTO]** Read Open Issues — Check if existing Issue exists
...
3. **[CALL]** Call `superpowers:brainstorming`
   - Pass context: Issue description (if exists) or user requirements
   - **⚠️ BRAINSTORMING RETURN RULE:** When called from this orchestrator,
     brainstorming's terminal state is **RETURN TO ORCHESTRATOR**, not
     `writing-plans`. After brainstorming writes the design doc and gets
     user approval, it MUST return here for the orchestrator to continue.
   - **brainstorming internally will:**
     ...
     - **Return control to orchestrator** (do NOT invoke `writing-plans`)
   - Output: `design_doc_path`

4. **[AUTO]** Call `gitflow-issue-create` — **MANDATORY**
   - Create GitHub Issue (or use existing Issue)
   - Reference design doc path in Issue body
   - Output: `issue_url`

5. **[AUTO]** Call `gitflow-issue-review` — **MANDATORY**
   - Review Issue quality
   - Add review comment
   - Output: `comment_id`

6. **[AUTO]** Update contract
   ```json
   phases.1.evidence = {
     "issue_url": "...",
     "comment_id": "...",
...
1. **[CALL]** Call `superpowers:writing-plans` to create a full plan
   - Input: design doc path (from `phases.1.evidence.design_doc_path`)
   - **⚠️ RETURN RULE:** After writing-plans completes, it MUST return
     control to this orchestrator. Do NOT let writing-plans chain to
...
| Error | Recovery |
|-------|----------|
| Contract file not found | Create new contract (start from Bootstrap) |
...
| Sub-skill did not return control | Reassert orchestrator state: read contract, resume at next step |
| Brainstorming chained to writing-plans | Interrupt: return to orchestrator, execute `gitflow-issue-create` next |

## Common Mistakes

❌ Skip gate check · ❌ Inline sub-skill · ❌ Advance to next Phase before contract update · ❌ worktree not cleaned · ❌ Yield to user skip requests · ❌ **Invoke sub-skill without creating contract first** · ❌ **Let brainstorming/writing-plans chain to next skill instead of returning to orchestrator** · ❌ **Skip Issue creation/review in Phase 1**
