...
封装 `gitflow-cli commit` 命令族，用于在 GitHub/GitLab/GitCode 等平台上查看 Commit 详情、获取 diff/patch 以及添加行内评论。
...
### `gitflow-cli commit view`

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `<sha>` | string | 是 | Commit 的 SHA 哈希值 |

### `gitflow-cli commit diff`

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `<sha>` | string | 是 | Commit 的 SHA 哈希值 |

### `gitflow-cli commit patch`

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `<sha>` | string | 是 | Commit 的 SHA 哈希值 |

### `gitflow-cli commit comment`
...
```bash
gitflow-cli commit view abc1234567890def
```
...
```bash
gitflow-cli commit diff abc1234567890def > commit-abc.diff
```

### 获取 Patch 并应用到本地仓库

```bash
gitflow-cli commit patch def0987654321fed | git apply -
```
