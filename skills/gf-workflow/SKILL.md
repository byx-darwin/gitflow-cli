...
| Step | Action | Output |
|------|--------|--------|
| 1 | **[AUTO]** Record `base_branch` via `git rev-parse --abbrev-ref HEAD`. Worktree path is FIXED at `.worktree/<branch-name>` (covered by `.worktree/` in `.gitignore`). Branch name: `feat/<issue-number>-<short-description>`. Created here for same-session mode; created by the executor (background agent / new window) otherwise — see `references.md` → Phase 3 Execution Modes. **After worktree creation**: symlink shared directories so workflow contracts and Claude config are accessible from the worktree: `mkdir -p <worktree-path>/.cache && ln -s $(pwd)/.cache/workflows <worktree-path>/.cache/workflows && ln -s $(pwd)/.claude <worktree-path>/.claude` | `branch`, `base_branch`, `worktree_path` |
