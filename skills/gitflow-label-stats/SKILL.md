...
对仓库中 Issue 的标签使用情况进行统计分析。通过调用 `gitflow-cli issue list --label` 按标签多次获取数据，结合 `gitflow-cli label list` 获取仓库全部标签定义，从标签分组计数、优先级分布、未分类 Issue 识别三个维度输出统计报告，帮助团队了解 Issue 管理健康度。
...
# 按主要标签统计 Open Issues
gitflow-cli issue list --label bug --state open
gitflow-cli issue list --label enhancement --state open
gitflow-cli issue list --label documentation --state open
...
```bash
# 1. 获取所有标签
gitflow-cli label list > /tmp/labels.txt

# 2. 获取所有 Open Issues
gitflow-cli issue list --state open --limit 1000 > /tmp/all-issues.txt

# 3. 逐个标签统计
while read -r LABEL; do
    COUNT=$(gitflow-cli issue list --label "$LABEL" --state open | wc -l)
    echo "$LABEL: $COUNT"
done < /tmp/labels.txt
...
- `gitflow-cli issue list --label` 支持多次使用进行 AND 过滤，但统计时应逐个标签独立统计
