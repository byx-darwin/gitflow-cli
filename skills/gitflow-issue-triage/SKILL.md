...
调用 `gitflow issue list` 获取当前所有 open 状态的 issues：

```bash
gitflow issue list --state open
```
...
对每个已完成分类的 Issue，调用 `gitflow issue label` 添加 `triage:done` 标签：

```bash
gitflow issue label <issue-number> --label "triage:done"
```

同时添加对应的类型和优先级标签：

```bash
gitflow issue label <issue-number> --label "type:bug" --label "priority:high"
```
...
gitflow issue label 10 --label "type:bug" --label "priority:urgent" --label "triage:done"
gitflow issue label 11 --label "type:feature" --label "priority:high" --label "triage:done"
gitflow issue label 12 --label "type:enhancement" --label "priority:medium" --label "triage:done"
gitflow issue label 13 --label "type:docs" --label "priority:low" --label "triage:done"
gitflow issue label 14 --label "type:bug" --label "priority:high" --label "triage:done"
gitflow issue label 15 --label "type:question" --label "priority:low" --label "triage:done"
gitflow issue label 16 --label "type:feature" --label "priority:medium" --label "triage:done"
gitflow issue label 17 --label "type:bug" --label "priority:medium" --label "triage:done"
...
```bash
# 先筛选最近创建的 issues
gitflow issue list --state open --since 2026-06-25

# 对筛选出的 issues 执行分类流程
gitflow issue label 20 --label "type:bug" --label "priority:high" --label "triage:done"
gitflow issue label 21 --label "type:feature" --label "priority:medium" --label "triage:done"
```
