//! GitHub Label 和 Milestone 提供者实现。
//!
//! 通过 `gh label` 和 `gh api` CLI 命令实现 [`LabelProvider`] 和
...
///
/// ```no_run
/// use gitflow_github::GitHubMilestoneProvider;
///
...
impl MilestoneProvider for GitHubMilestoneProvider {
    ...
    async fn edit(&self, number: u64, args: CreateMilestoneArgs) -> Result<MilestoneData> {
        debug!(repo = %self.repo, number, "spawning `gh api milestones PATCH`");
        ...
        let output = cmd.output().await.map_err(|e| {
            CoreError::Platform(format!("Failed to spawn gh api milestone edit: {e}"))
        })?;
    ...
    async fn close(&self, number: u64) -> Result<MilestoneData> {
        debug!(repo = %self.repo, number, "spawning `gh api milestones close`");
        ...
        let output = tokio::process::Command::new("gh")
            ...
            .await
            .map_err(|e| {
                CoreError::Platform(format!("Failed to spawn gh api milestone close: {e}"))
            })?;
    ...
    async fn reopen(&self, number: u64) -> Result<MilestoneData> {
        debug!(repo = %self.repo, number, "spawning `gh api milestones reopen`");
        ...
        let output = tokio::process::Command::new("gh")
            ...
            .await
            .map_err(|e| {
                CoreError::Platform(format!("Failed to spawn gh api milestone reopen: {e}"))
            })?;
...
mod tests {
    ...
    #[test]
    fn test_should_construct_github_label_provider() {
        let provider = GitHubLabelProvider::new("octocat/hello-world");
        assert_eq!(provider.repo, "octocat/hello-world");
    }

    #[test]
    fn test_should_construct_github_label_provider_from_string() {
        let repo = String::from("octocat/hello-world");
        let provider = GitHubLabelProvider::new(repo);
        assert_eq!(provider.repo, "octocat/hello-world");
    }

    #[test]
    fn test_should_debug_format_label_provider() {
        let provider = GitHubLabelProvider::new("owner/repo");
        let debug = format!("{provider:?}");
        assert!(debug.contains("GitHubLabelProvider"));
        assert!(debug.contains("owner/repo"));
    }

    #[test]
    fn test_should_clone_github_label_provider() {
    ...
    #[test]
    fn test_should_construct_github_milestone_provider() {
        let provider = GitHubMilestoneProvider::new("octocat/hello-world");
        assert_eq!(provider.repo, "octocat/hello-world");
    }
    ...
    fn test_should_debug_format_milestone_provider() {
        let provider = GitHubMilestoneProvider::new("owner/repo");
        let debug = format!("{provider:?}");
        assert!(debug.contains("GitHubMilestoneProvider"));
        assert!(debug.contains("owner/repo"));
    }

    #[test]
    fn test_should_clone_github_milestone_provider() {
        let original = GitHubMilestoneProvider::new("owner/repo");
        let cloned = original.clone();
        assert_eq!(original.repo, cloned.repo);
    }
    ...
    #[test]
    fn test_should_deserialize_label_data_from_gh_output() {
        let json = br#"[
    ...
    #[test]
    fn test_should_deserialize_single_label_from_gh_output() {
