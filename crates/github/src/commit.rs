//! GitHub Commit 提供者实现。
//!
//! 通过 `gh api` CLI 命令实现 [`CommitProvider`] trait，
...
/// GitHub Commit 提供者，通过 `gh api` 查看提交信息。
///
/// # Examples
///
/// ```no_run
...
/// ```
#[derive(Debug, Clone)]
pub struct GitHubCommitProvider {
    /// GitHub `owner/repo`。
    repo: String,
}

impl GitHubCommitProvider {
    /// 创建新的 GitHub Commit 提供者。
    ///
    /// `repo` 格式为 `owner/repo`。
    #[must_use]
...
/// 为 `CommitData` 构造后备用户（当 API 未返回用户信息时）。
...
#[async_trait]
impl CommitProvider for GitHubCommitProvider {
    async fn view(&self, sha: &str) -> Result<CommitDetail> {
        debug!(repo = %self.repo, sha, "spawning `gh api commit view`");
    ...
    async fn diff(&self, sha: &str) -> Result<String> {
        debug!(repo = %self.repo, sha, "spawning `gh api commit diff`");
    ...
    async fn patch(&self, sha: &str) -> Result<String> {
        debug!(repo = %self.repo, sha, "spawning `gh api commit patch`");
        ...
        let output = tokio::process::Command::new("gh")
            .args(["api", &api_path])
            .arg("-H")
            .arg("Accept: application/vnd.github.v3.patch")
            .output()
            .await
            .map_err(|e| {
                CoreError::Platform(format!("Failed to spawn gh api commit patch: {e}"))
            })?;
    ...
    async fn comment(&self, sha: &str, body: &str, path: &str, line: u64) -> Result<()> {
        debug!(repo = %self.repo, sha, "spawning `gh api commit comment`");
        ...
        let output = cmd.output().await.map_err(|e| {
            CoreError::Platform(format!("Failed to spawn gh api commit comment: {e}"))
        })?;
...
mod tests {
    ...
    // --- GitHubCommitProvider tests ---

    #[test]
    fn test_should_construct_github_commit_provider() {
        let provider = GitHubCommitProvider::new("octocat/hello-world");
        assert_eq!(provider.repo, "octocat/hello-world");
    }

    #[test]
    fn test_should_construct_github_commit_provider_from_string() {
        let repo = String::from("octocat/hello-world");
        let provider = GitHubCommitProvider::new(repo);
        assert_eq!(provider.repo, "octocat/hello-world");
    }

    #[test]
    fn test_should_debug_format_commit_provider() {
        let provider = GitHubCommitProvider::new("owner/repo");
        let debug = format!("{provider:?}");
        assert!(debug.contains("GitHubCommitProvider"));
        assert!(debug.contains("owner/repo"));
    }

    #[test]
    fn test_should_clone_github_commit_provider() {
        let original = GitHubCommitProvider::new("owner/repo");
