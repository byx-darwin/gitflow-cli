...
///
/// Cloning shares the same queue via `Arc<Mutex<...>>`, so clones advance together.
#[cfg(test)]
#[derive(Debug, Clone)]
...
impl SequencedMockCommandRunner {
    ...
    /// # Panics
    ///
    /// The runner panics at runtime if `run` is called more times than there are
    /// entries in `outputs`.
    #[must_use]
    ...
    /// Convenience: build a sequence from a list of `(success, stdout_or_stderr)` tuples.
    ///
...
mod tests {
    ...
    async fn test_should_yield_sequenced_responses() {
        ...
        let out2 = runner.run("gh", &[]).await.expect("second should 'succeed' as Output");
