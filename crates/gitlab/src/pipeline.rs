...
///
/// ```no_run
/// use gitflow_gitlab::GitLabPipelineProvider;
///
...
pub struct GitLabPipelineProvider<R: CommandRunner = RealCommandRunner> {
    ...
    repo: String,
    /// 传给 `glab ci ...` 子命令 `--repo` 参数的目标字符串。默认等于 `repo`；
    /// 通过 [`with_remote_url`](GitLabPipelineProvider::with_remote_url) 构造时
    /// 为完整 git remote URL，用于在自建 GitLab 实例上显式锁定 host。
    repo_target: String,
...
impl<R: CommandRunner + 'static> PipelineProvider for GitLabPipelineProvider<R> {
    ...
    async fn logs(&self, pipeline_id: u64) -> Result<String> {
        ...
        let output = self
            ...
            .run(
                "glab",
                &["ci", "trace", &id_str, "--repo", &self.repo_target],
            )
            .await
...
mod tests {
    ...
    #[tokio::test]
    async fn test_should_use_explicit_repo_target_for_logs() {
        ...
        let result = provider.logs(99).await;
