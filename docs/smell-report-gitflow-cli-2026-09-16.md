...
Stage 1 的 clippy 命令**只运行一次**并捕获到文件后复用。本次运行输出 782 行，包含
`Compiling gitflow-cli v1.9.0` 编译行，且 12 个 target 的诊断均完整回放，因此不是缓存空跑。
...
| `dead_code` | 79 | —— | Stage 2 后全部落入已排除，见下 |
...
| Dead Code | 33 处（5 个 lib crate + `release-signer`） | —— | —— | `dead_code` |
| Dead Code | 46 处（`apps/cli`，`bin "gf" test` target） | —— | —— | `dead_code` |
...
- **关于抑制的判定**：5 处**全部**带 `#[allow(clippy::too_many_lines, reason = "Command dispatch: each match arm maps to one operation")]`，属「有理由的抑制」。逐条核对该理由是否仍然成立后判定：理由准确描述了分发骨架，但未覆盖上述 4 个含内联解析逻辑的臂；因此不整条移入已排除，而是保留为 Low 级 finding，供维护者复核理由与实际形态的偏差。另有 3 处同款抑制（`auth.rs:49`、`review.rs:127`、`commit.rs:76`）本次未越界，不产生候选
...
| B | `apps/cli/src/**` 46 处 `function ... is never used`（含 10 处 `handle`、`main`、`async_main`、`router` 等） | 检测产物 | 这些诊断来自 `bin "gf" test` target：`--all-targets` 会把二进制 crate 再编译为测试 harness，此时 `main` 及其调用链整体不可达。已直接阅读源码确认可达性，例如 `apps/cli/src/main.rs:178` 调用 `commands::pr::handle(...)`。非生产二进制中的死代码 |
| B | `crates/release-signer/src/main.rs` 4 处 `never used` | 检测产物 | 同上，来自 `bin "release-signer" test` target |
...
- 阈值越界 ≠ finding：11 类候选进入 Stage 1，最终只有 3 条成为 finding。18 个文件行数候选存活 1 条，79 条 `dead_code` 存活 0 条
