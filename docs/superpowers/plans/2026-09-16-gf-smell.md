...
Expected: FAIL，输出 `✗ missing skills/gf-smell/SKILL.md`，**非零退出**。

注意：GNU Make 3.81 会把任何失败的 recipe 统一包装为 `Error 1` 并以进程退出码 **2**
结束，而 recipe 自身的 `exit 1` 仍正常触发。断言的契约是「非零退出 + 打印 `✗` 消息」，
不要按「退出码恰为 1」去核对。
