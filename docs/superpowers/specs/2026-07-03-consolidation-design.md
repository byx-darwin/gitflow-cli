# gf 合并 ncgo-code-skills 与 gitcode-dev-workflow 设计
...
- **gf**（Rust CLI + 27 skills）：底层 CLI 引擎 + 完整 skill 集，支持 GitHub/GitLab/GitCode 三平台
...
1. **gf 为唯一仓库** — 合并后所有 skill/hook 在 gf 维护
2. **取增量不取存量** — 只吸收源项目中 gf 没有或明显更好的部分
3. **保持实现习惯** — weekly-report 保持纯 bash，不重写为 Rust；workflow 保持 Markdown + CLI 指挥模式
4. **不破坏现有行为** — 变更对 gf 现有 27 个 skill 无副作用
...
受影响的方法：步骤 1.4、2.5、3.3、4.6 中所有 `gf issue comment <number> --body "..."` 的长内容调用。
...
gf 现有安装系统已支持 `skills install -g`（安装到 `~/.claude/skills/`），不需要修改安装机制。在 SKILL.md 描述中明确推荐 `-g` 安装即可。
...
| 能力 | 采用方 | 原因 |
|------|--------|------|
| Issue 创建 / 去重搜索 | gf 版 | `gf` CLI 天然三平台统一 |
| pending.json schema | gf 版 | 已有完整字段定义（error_id / command / platform / error_code / error_message / timestamp） |
| hooks 触发机制 | ncgo-code 版 | Stop Hook 检测 pending.json 更可靠 |
| auth cache（24h TTL） | ncgo-code 版 | 避免每次上报都调用 `gf auth status` |
...
Claude 分析错误上下文 → 生成 Issue 标题/正文
    ↓
gf issue create --title "..." --body "..." --label "auto-report"
    ↓ 成功
清理 pending.json
```
...
ncgo-code 的 `auto-smoke-test.sh` 监控其 bash `_provider.sh` 脚本变更。gf 没有 provider 抽象层（由 Rust core 替代），该 hook 不适用。
...
| weekly-report 的 bash 与 gf Rust 风格不一致 | weekly-report 是独立 skill，bash 是刻意选择（零依赖），不影响其他 skill |
| sync-readme-check.sh 的目录检测逻辑需适配 gf 结构 | 实现时以 gf 当前 skills/ 目录结构为准 |
