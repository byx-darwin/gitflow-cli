...
async fn async_main(cli: Cli, platform: &str, repo: &str, remote_url: &str) -> miette::Result<()> {
    ...
    if !matches!(
        ...
        Commands::Skills(_)
            | Commands::Decide(_)
            | Commands::Regression(_)
            | Commands::Issue(IssueCommand::Precheck { .. } | IssueCommand::Search(_))
            | Commands::Pr(PrCommand::Precheck { .. } | PrCommand::Search(_))
            | Commands::Pipeline(PipelineCommand::AnalyzeFailures { .. })
            | Commands::Completions(_)
            | Commands::Workflow(_)
            | Commands::Update(_)
