...
fn print_toon<T: serde::Serialize>(value: &T) -> miette::Result<()> {
    ...
    let toon = gitflow_core::toon::encode(&json_value)
...
fn print_auto<T: serde::Serialize>(value: &T) -> miette::Result<()> {
    ...
    let shape = gitflow_core::toon::analyze(&json_value);
    let strategy = gitflow_core::toon::select_strategy(&shape);

    match strategy {
        gitflow_core::toon::ToonStrategy::Toon => {
            let toon = gitflow_core::toon::encode(&json_value)
                .map_err(|e| miette::miette!("Failed to encode TOON output: {e}"))?;
            println!("{toon}");
        }
        gitflow_core::toon::ToonStrategy::Json => {
